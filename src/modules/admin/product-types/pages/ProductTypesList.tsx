...
export default function ProductTypesList() {
  ...
  const { items, loading, error, toggle, remove, refetch } = useProductTypes(debouncedSearch)
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('¿Seguro que querés eliminar este tipo de producto?')) {
      return
    }
    try {
      await remove(id)
      setFeedbackType('success')
      setFeedback('Tipo de producto eliminado')
    } catch (err) {
      console.error(err)
      setFeedbackType('error')
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Tipos de producto</h1>
          <p className="text-sm text-slate-500">Clasificá tus productos para organizar el catálogo.</p>
        </div>
        <button
          onClick={() => history.push('/admin/tipos-producto/nuevo')}
                  ...
                  <tr key={item.id} className="transition hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2 text-xs">
                        <button
                          onClick={() => history.push(`/admin/tipos-producto/${item.id}`)}
