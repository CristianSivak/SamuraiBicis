...
export default function ProductTypes() {
  ...
  async function handleSubmit(data) {
    try {
      if (editing?.id) {
        await updateProductType(editing.id, data);
      } else {
  ...
  async function handleDelete(row) {
    if (!confirm(`¿Eliminar el tipo "${row.title}"?`)) return;
    try {
      await removeProductType(row.id);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-8 shadow-[0_45px_80px_-50px_rgba(15,23,42,0.2)]">
        <div className="absolute -right-28 top-1/2 h-64 w-64 -translate-y-1/2 rounded-full bg-indigo-500/25 blur-3xl" />
        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-3">
            ...
            <div>
              <h1 className="text-3xl font-semibold text-slate-900">Tipos de producto</h1>
              <p className="mt-3 max-w-xl text-sm text-slate-500">
                Crea y organiza categorías de productos para reutilizarlas al cargar tu inventario.
              </p>
            </div>
          </div>
          <button
            className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-indigo-500 to-purple-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-200/60 transition hover:-translate-y-0.5"
            onClick={() => {
              setEditing(null);
              setModalOpen(true);
            }}
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_35px_65px_-45px_rgba(15,23,42,0.18)]">
        <input
          className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-700 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/40"
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm text-slate-600">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td className="px-4 py-12 text-center text-slate-500" colSpan={2}>
              ...
              <tr key={row.id}>
                ...
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-2">
                    <button
                      className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-600 transition hover:border-indigo-400 hover:text-slate-900"
                      onClick={() => {
                        setEditing(row);
                        setModalOpen(true);
                      }}
...
function ProductTypeModal({ open, onClose, onSubmit, initial }) {
      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
        ...
        >
          <div className="space-y-2">
            ...
            <input
              className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/40"
              value={title}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              disabled={saving}
              className="rounded-2xl bg-gradient-to-r from-indigo-500 to-purple-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-200/60 transition focus:outline-none focus:ring-2 focus:ring-indigo-400/60 disabled:opacity-60"
            >
